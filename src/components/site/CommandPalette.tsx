    ...
    <div className="fixed inset-0 z-[80] flex items-start justify-center px-4 pt-[14vh]" role="dialog" aria-modal="true" aria-label="Command palette">
      ...
      <div className="palette-in relative w-full max-w-[640px] overflow-hidden rounded-[20px] border border-black/5 bg-[var(--nile-card)] shadow-[0_24px_80px_-24px_rgba(0,0,0,0.45)]">
        <div className="flex items-center gap-3 border-b border-black/5 px-5 py-4">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="size-[18px] text-[#6E6E73]">
          ...
          />
          <kbd className="rounded-[6px] border border-black/10 px-1.5 py-0.5 text-[10px] text-[#6E6E73]">ESC</kbd>
        </div>
        <div className="max-h-[52vh] overflow-y-auto p-2">
          {results.length === 0 && (
            <p className="px-4 py-8 text-center text-[13px] text-[#6E6E73]">No matches. Try “SEO” or “pricing”.</p>
          )}
          {results.map((r, i) => {
            ...
            return (
              <div key={`${r.group}-${r.label}`}>
                {header && (
                  <p className="px-3 pb-1 pt-3 text-[10.5px] font-semibold uppercase tracking-[0.16em] text-[#6E6E73]">{header}</p>
                )}
                <button
                ...
                >
                  <span className="min-w-0">
                    <span className="block truncate text-[14px] font-medium text-[#1D1D1F]">{r.label}</span>
                    <span className="block truncate text-[12.5px] text-[#6E6E73]">{r.hint}</span>
                  </span>
                  <span aria-hidden className="text-[13px] text-[#6E6E73]">↵</span>
                </button>
              </div>
            );
          })}
